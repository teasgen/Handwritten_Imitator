...
class AdversarialModel(BaseModel):
    ...
    def eval_text_custom_image(self, org_img, text):
        ...
        with torch.no_grad():
            ...
            # real_imgs = (1 - real_img).squeeze().cpu().numpy() * 127
            gen_imgs = self.models.G(enc_styles, fake_lbs, fake_lb_lens)
            ...
            # plt.figure()
            # plt.subplot(nrow * 2, 1, 1)
            # plt.imshow(real_imgs[:, :real_img_len], cmap='gray')
            # plt.axis('off')
            #
            # plt.subplot(nrow * 2, 1, 2)
        ...
        for epoch in range(epoch_done, self.opt.training.epochs):
            for i, batch in enumerate(self.train_loader):
                ...
                if iter_count % self.opt.training.num_critic_train == 0:
                    ...
                    set_requires_grad([self.models.D, self.models.P, self.models.R, self.models.W, self.models.B],
                                      False)
                    ...
                    grad_fake_adv = torch.autograd.grad(adv_loss, cat_fake_imgs, create_graph=True, retain_graph=True)[
                        0]
                    grad_fake_OCR = \
                        torch.autograd.grad(fake_ctc_loss_rand, fake_ctc_rand, create_graph=True, retain_graph=True)[0]
                    grad_fake_info = torch.autograd.grad(info_loss, fake_imgs, create_graph=True, retain_graph=True)[0]
                    grad_fake_wid = \
                        torch.autograd.grad(fake_wid_loss, recn_wid_logits, create_graph=True, retain_graph=True)[0]
...
class RecognizeModel(BaseModel):
    ...
    def train(self):
        ...
        trainset_info = (
            self.opt.training.dset_name, self.opt.training.dset_split, False, self.opt.training.augment, True)
